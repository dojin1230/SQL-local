...
-- 신용카드 변경 동의자 확인 -- TM에서 신용카드 변경 콜 성공 후 카드변경을 했는지 확인
SELECT *
...
LEFT JOIN
  ...
  WHERE 승인일 >= CONVERT(varchar(10), DATEADD(day, -4, GETDATE()), 126)
) CA
ON H.회원번호 = CA.회원번호
WHERE 기록분류 = 'TM_신용카드종료예정' AND 기록분류상세 = '통성-변경동의'
  AND H.참고일 >= CONVERT(varchar(10), DATEADD(day, -4, GETDATE()), 126)
  AND CA.회원번호 IS NULL
...
LEFT JOIN
	...
	WHERE 기록분류 = 'TM_감사' AND 처리진행사항 = 'SK-진행') H
ON D.회원번호 = H.회원번호
WHERE
	CMS증빙자료등록필요 = 'Y'
	AND 회원상태 =  'Normal'
	AND 가입일 < CONVERT(varchar(10), DATEADD(day, -3, GETDATE()), 126)   -- 최초등록일로 수정??
	AND H.회원번호 IS NULL
	AND D.최초등록구분 != '일시'
