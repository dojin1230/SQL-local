...
/*
	1-1. EN 원본데이터 추출 TRANSACTION - csv로 저장후 [Import flat file]로 가져오기 
		대상:[dbo].[en_data]
	1-2. FB 원본데이터 추출: 정연샘에게 csv파일 받아서 [Import flat file]로 가져오기 
		대상:[dbo].[fb_data]
	2. DB0_clnt_i 가져오기
		대상:[dbo].[db0_clnt_i]
	3. petition_all 가져오기
...
SELECT 
	...
	REPLACE(REPLACE([work].[dbo].[fb_data].[phone_number],'p:',''),'+82','') AS phone_number_1,
...
WHERE 
	[work].[dbo].[fb_data].[개인정보_이용에_동의합니다_] = N'예'
	AND
	[work].[dbo].[fb_data].[개인정보_제3자_이용에_동의합니다_] = N'예'


-- 캠페인 날짜 형식 수정하고 이틀 지나간 데이터만 추출 -- 

SELECT *
INTO #001_2
FROM
	(SELECT 
		[work].[dbo].[fb_001].[supporter_email], 
		[work].[dbo].[fb_001].[korean_name], 
		[work].[dbo].[fb_001].[phone_number], 
		[work].[dbo].[fb_001].[phone_number_1],
		[work].[dbo].[fb_001].[campaign_id],
		SUBSTRING(([work].[dbo].[fb_001].[date_time]),1,10) AS campaign_date,
		[work].[dbo].[fb_001].[datapart], 
		[work].[dbo].[fb_001].[source]

	FROM [work].[dbo].[fb_001]) fb_tmp_01

WHERE 
	fb_tmp_01.[campaign_date] <= GETDATE() - 2

-- 임시테이블 열 갯수 맞추고 전화번호 앞 + 기호 제거 --
