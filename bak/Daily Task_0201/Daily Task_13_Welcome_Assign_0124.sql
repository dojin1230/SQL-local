...
WHERE
	...
	AND (
		...
		OR (D.CMS상태 = '수정대기' AND D.CMS증빙자료등록필요 = 'Y')	-- CMS대기중이고 증빙자료 없는 경우
		OR (D.CMS상태 = '신규완료')
		OR (D.CMS상태 = '수정완료')									-- 또는 CMS완료인 경우
		OR (D.CMS상태 = '신규실패')
		OR (D.CMS상태 = '수정실패')									-- 또는 CMS실패인 경우
		OR (D.납부방법 = '신용카드')								-- 또는 신용카드인 경우
		)
ORDER BY 2 DESC