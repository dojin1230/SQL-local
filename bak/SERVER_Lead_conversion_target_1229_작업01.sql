...
WHERE [work].[dbo].[en_data].[Campaign_Data_2] = 'Kim'


-- 작업 001 -- EN에서 추출한 데이터에서 따옴표 제거 및 출처정보를 [data1]에 취합 -- REPLACE 제거 가능
...
-- 페이스북 리드 캠페인 날짜 형식 수정하고 이틀 지나간 데이터만 추출 -- 
...
-- 페이스북 리드 임시테이블 열 갯수 맞추고 전화번호 앞 + 기호 제거 --
...
--  작업 003-1 -- 180일 이내에 [petition_all]에 기록된 사람 제외 (휴대폰번호/수정된 휴대폰번호/이메일 기준)
