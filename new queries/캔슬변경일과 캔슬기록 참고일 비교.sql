...
LEFT JOIN 
	(SELECT * FROM MRMRT.그린피스동아시아서울사무소0868.dbo.UV_GP_관리기록 
	WHERE 기록분류 = 'Cancellation' AND 참고일 >= GETDATE() - 9) H			-- 변경일시에 지정한 숫자보다 1 큰 숫자 입력
ON CH.회원번호 = H.회원번호
WHERE 수정후 = 'canceled'													-- 회원상태가 canceled로 변경된 자료검색
	AND 변경일시 >= GETDATE() - 8											-- 오늘로부터 24*8 시간전 변경된 자료까지 검색 (숫자 변경 가능)
	AND H.회원번호 IS NULL
